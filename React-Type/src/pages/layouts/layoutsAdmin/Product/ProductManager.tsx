            ...
            <div className=" flex justify-start items-center py-0 relative">
              ...
              <svg className="absolute right-3 z-10 cursor-pointer" width={24} height={24} viewBox="0 0 24 24" fill="none"
                ...
                <path d="M21 21L15 15" stroke="#4B5563" strokeWidth="1.66667" strokeLinecap="round" strokeLinejoin="round" />
                  ...
                  <button className="transition focus:outline-none duration-150 ease-in-out hover:bg-indigo-600 bg-indigo-700 rounded text-white px-8 py-2 mt-2 text-sm"><Link to={"add"} >  Add More Prodcut</Link></button>
          </div>
        </div>
      </div>
          ...
          {props.products.map((item, index) => {
            return <tr className="border-b dark:bg-gray-800 dark:border-gray-700 odd:bg-white even:bg-gray-50 odd:dark:bg-gray-800 even:dark:bg-gray-700">
              ...
              <td className="px-6 py-4 text-right">
                <button><Link to={`/admin/product/${item._id}/edit`} className="transition focus:outline-none duration-150 ease-in-out hover:bg-indigo-600 bg-indigo-700 rounded text-white px-8 py-2 mt-2 text-sm">Edit</Link></button>
              </td>
              <td className="px-6 py-4 text-left">
                <button type='submit' onClick={() => props.onRemove(item._id)} className="transition focus:outline-none duration-150 ease-in-out hover:bg-indigo-600 bg-indigo-700 rounded text-white px-8 py-2 -ml-6 mb-1 mt-2 text-sm">Remove</button>
              </td>
            </tr>
          })}
        </tbody>
